...
impl CostBasedOptimizer {
    ...
    fn estimate_where_cost(&self, where_clause: &WhereCondition, match_clause: &Option<MatchClause>) -> Cost {
        match where_clause {
            WhereCondition::PropertyFilter { property, operator: _, value } => {
            ...
            WhereCondition::ToroidalDistance { field: _, threshold } => {
                // Vector search cost depends on threshold
    ...
    fn estimate_selectivity(&self, property: &str, value: &PropertyValue, match_clause: &Option<MatchClause>) -> f64 {
    ...
    fn estimate_graph_traversal_cost(&self, query: &Query) -> Cost {
        let max_hops = query.within_clause
            .as_ref()
            .map(|w| w.max_hops)
            .unwrap_or(2);
    ...
    fn estimate_result_cardinality(&self, query: &Query) -> f64 {
