...
impl GraphAnalytics {
        ...
        for s_node in &all_nodes {
            ...
            for t_node in &all_nodes {
                ...
                let paths_through_node = paths.iter()
                    .filter(|path| path.contains(&node_id))
                    .count() as f32;
    ...
    fn eigenvector_centrality(
        ...
        let node_index: HashMap<u64, usize> = all_nodes.iter()
            .enumerate()
        ...
        for _ in 0..100 {
            ...
            // Normalization
            let norm: f32 = new_centrality.iter()
                .map(|&x| x * x)
                .sum::<f32>()
                .sqrt();
    ...
    fn louvain(
        ...
        let mut communities: HashMap<u64, u32> = all_nodes.iter()
        ...
        for _ in 0..10 {
            for node in &all_nodes {
                let neighbors = store.get_neighbors(node.id)?;
                ...
                if let Some((&best_comm, _)) = community_gain.iter()
    ...
    fn label_propagation(
        ...
        let mut labels: HashMap<u64, u32> = all_nodes.iter()
            .map(|node| (node.id, node.id as u32))
            .collect();
        ...
        for _ in 0..20 {
            for node in &all_nodes {
                ...
                if let Some((&best_label, _)) = label_freq.iter()
    ...
    fn bfs_distance(
        ...
        while let Some((current, distance)) = queue.pop_front() {
            ...
            let neighbors = store.get_neighbors(current)?;
            for neighbor in neighbors {
                if !visited.contains(&neighbor.id) {
                    visited.insert(neighbor.id);
    ...
    fn find_all_shortest_paths(
        ...
        Self::dfs_paths(store, start, end, &mut visited, &mut current_path, &mut paths)?;
...
mod tests {
    ...
    fn create_test_graph() -> Hyb ridPersistentStore {
        ...
        // Добавляем рёбра
        for i in 1..4 {
            store.add_edge(i, i + 1, "CONNECTS".to_string(), 1.0).unwrap();
            store.add_edge(i + 1, i, "CONNECTS".to_string(), 1.0).unwrap();
        }
    ...
    fn test_degree_centrality() {
        ...
        let centrality = GraphAnalytics::centrality(&store, 3, CentralityAlgorithm::Degree).unwrap();
