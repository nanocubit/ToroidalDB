...
impl BiConnector {
    ...
    fn connect_to_metabase(&self) -> Result<BiConnection, String> {
        ...
        Ok(BiConnection {
            ...
            connected: true,
            connection_info: format!("Connected to Metabase at: {}", self.config.connection_string),
        })
    }
    ...
    fn connect_to_superset(&self) -> Result<BiConnection, String> {
        ...
        Ok(BiConnection {
            ...
            connected: true,
            connection_info: format!("Connected to Superset at: {}", self.config.connection_string),
        })
    }
    ...
    fn connect_custom(&self) -> Result<BiConnection, String> {
        ...
        Ok(BiConnection {
            ...
            connected: true,
            connection_info: format!("Connected to custom BI at: {}", self.config.connection_string),
        })
    }
    ...
    pub fn export_json(&self, query: &str) -> Result<String, String> {
        let bi_data = self.export_for_bi(query)?;
        serde_json::to_string(&bi_data)
            .map_err(|e| format!("Failed to serialize BI data: {}", e))
    }
        ...
        for node in &all_nodes {
            ...
            if let Some(props) = node.properties.as_object() {
                for (key, value) in props {
                    ...
                    property_types.entry(key.clone())
...
impl TableauConnector {
    ...
    pub fn prepare_tableau_data(&self, nodes: &[Node]) -> Result<String, String> {
        ...
        for node in nodes {
            let label = node.properties.get("label")
                .and_then(|v| v.as_str())
                .unwrap_or("unknown");
            let score = node.properties.get("score")
                .and_then(|v| v.as_f64())
                .unwrap_or(0.0);
...
mod tests {
    ...
    fn test_bi_connector_creation() {
        let store = Arc::new(HybridPersistentStore::open("./test_bi_data").unwrap());
    ...
    #[test]
    fn test_schema_info() {
        let store = Arc::new(HybridPersistentStore::open("./test_bi_data2").unwrap());
        let config = BiConnectorConfig {
            connector_type: BiConnectorType::Tableau,
    ...
    #[test]
    fn test_csv_export() {
        let store = Arc::new(HybridPersistentStore::open("./test_bi_data3").unwrap());
        let config = BiConnectorConfig {
            connector_type: BiConnectorType::Metabase,
    ...
    fn test_connection_types() {
        let store = Arc::new(HybridPersistentStore::open("./test_bi_data4").unwrap());
